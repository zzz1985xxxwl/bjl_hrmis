...
namespace SEP.Performance.Pages.HRMIS.PayModulePages
{
	...
	public class SetEmployeeSalaryHandler : IHttpHandler, IRequiresSessionState
	{
		...
		private void Export()
		{
			...
			NowDate = NowDate.Replace("-", "");
			NowDate = NowDate.Replace("/", "");
			string filename = "员工薪资表" + NowDate + ".csv";
			Export2(filename);
		}
		...
		private void Export2(string FileName)
		{
			...
			_Context.Response.AddHeader("Content-Length", stream.Length.ToString());
			_Context.Response.ContentType = "application/CSV";
			_Context.Response.Write(stream);
