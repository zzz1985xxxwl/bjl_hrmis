...
// All rights reserved.
// 文件名: UpdateCompanyGoalPresenter.cs
// 创建者: 王玥琦
// 创建日期: 2008-06-05
// 概述: 更新公司目标
// ----------------------------------------------------------------
...
{
    public class UpdateCompanyGoalPresenter : ShowCompanyGoalPresenter
    {
        public EventHandler _CompleteEvent;

        public UpdateCompanyGoalPresenter(IGoalBaseView view, Account loginUser)
            : base(view, loginUser)
        {
        }
        ...
        public void ExecuteEvent(object sender, EventArgs e)
        {
            if (Validation())
            {
                CompanyGoal companyGoal =
                    new CompanyGoal(Convert.ToInt32(_IGoalBaseView.GoalID),
                                    _IGoalBaseView.Title, _IGoalBaseView.Content,
                                    Convert.ToDateTime(_IGoalBaseView.SetTime));
                try
                {
                    BllInstance.GoalBllInstance.UpdateCompanyGoal(companyGoal, LoginUser);
