...
namespace Framework.Core
{
    ...
    public sealed class CHS2PinYin
    {
        ...
        /// 汉字取拼音首字母大写
        /// </summary>
        public static string FirstCHSCap(string chs)
        {
            try
            {
                string letter = Convert(chs[0].ToString(), true)[0].ToString().Trim().ToUpper();
                char l = letter[0];
                if (l < 'A' || l > 'Z')
                {
                    throw new Exception("无法取得第一个大写字母！");
                }
                return letter;
            }
            catch
